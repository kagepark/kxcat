...
network() {
    ip_arr=($(echo $1 | sed "s/\./ /g"))
    netm_arr=($(echo $2 | sed "s/\./ /g"))
    chk=0
    for ((ii=0; ii<${#ip_arr[*]}; ii++)); do
       [ "${netm_arr[$ii]}" == "0" ] && break
       [ "$chk" == "0" ] && chk=1 || printf "."
       printf "%d" "$((${ip_arr[$ii]} & ${netm_arr[$ii]}))"
    done
}
...
#  _k_servicectl ntpd on 35
  ...
  # DNS config
  for ii in /etc/named.conf /var/named; do
      if [ ! -f ${ii}.ORIG -a ! -d ${ii}.ORIG ]; then
          [ -f "$ii" -o -d "$ii" ] && cp -a $ii ${ii}.ORIG
      fi
  done
  
  echo "#generated by xCAT: /opt/xcat/sbin/makedns command 
options {
	directory \"/var/named/\";
	allow-recursion { any; };
	forwarders {
...
key xcat_key {
        algorithm hmac-md5;
        secret \"SjNqekQ3eWN2ZEVsek9QR3RiNkJDSVpUaFprTDF1QjU=\";
};
" > /etc/named.conf

  for ii in $GROUP_NETWORK $MPI_NETWORK; do
  network_ip=$(network $ii $GROUP_NETMASK)
  network_ip_rev=$(rev_ip $network_ip)
    echo "
zone \"${network_ip_rev}.IN-ADDR.ARPA.\" in {
	type master;
	allow-update {
		key xcat_key;
	};
  ...
  echo "\$ORIGIN .
\$TTL 86400	; 1 day
${network_ip_rev}.IN-ADDR.ARPA	IN SOA	${MGT_HOSTNAME}.${DOMAIN_NAME}. root.${MGT_HOSTNAME}.${DOMAIN_NAME}. (
...
" >> /etc/named.conf
  ...
  echo "\$ORIGIN .
\$TTL 86400	; 1 day
${DOMAIN_NAME}		IN SOA	${MGT_HOSTNAME}.${DOMAIN_NAME}. root.${MGT_HOSTNAME}.${DOMAIN_NAME}. (
				$(date +"%Y%m%d")01 ; serial
				10800      ; refresh (3 hours)
				3600       ; retry (1 hour)
				604800     ; expire (1 week)
